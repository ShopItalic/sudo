...
@REM 校验DFU标志字符串的格式
set HARDWARE_BATH_VALID="#define HARDWARE_1231_ENABLED"
...
@REM 判断宏定义值是否等于字符串 "1"
if "%check%"=="1" (
    echo HARDWARE_1231_ENABLED is defined as 1
) else (
    echo HARDWARE_1231_ENABLED is not defined as 1
    echo %check%
    exit
)
...
@REM 软件版本字符串的格式
set SOFTWARE_VERSION="#define RING_1232_ONE_SEC_SOFTWARE_VERSION"

@REM 硬件版本字符串的格式
set HARDWARE_VERSION="#define RING_1232_ONE_SEC_HARDWARE_VERSION"
...
mergehex.exe -m ..\..\..\dfu\softdevice_boot\s140_nrf52_7.2.0_softdevice.hex ..\..\..\dfu\softdevice_boot\s2p_mac_fd3_boot_no_reset_nfc.hex ..\..\..\dfu\ota_bat\settings.hex .\Objects\app.hex -o ..\..\..\dfu\ota_output\BCL603S2P_%software_version%.hex
