...
static uint8_t CurrentVal[3] = {10,10,10};
...
static struct gh3026_g_sensor_struct g_sensor_struct = {NULL,NULL,NULL};
...
void ppg_current_val_set(uint8_t CurrentVal_0,uint8_t CurrentVal_1,uint8_t CurrentVal_2)
{
	CurrentVal[0] = CurrentVal_0;
	CurrentVal[1] = CurrentVal_1;
	CurrentVal[2] = CurrentVal_2;
}
...
void hal_gh3x2x_i2c_deinit(void)
{
...
//    GOODIX_PLATFORM_am_bsp_i2c_write_ENTITY(device_id, write_buffer,length);
    bc_ppg_i2c_write(device_id,0,(uint8_t*)write_buffer,length);
...
//    GOODIX_PLATFORM_I2C_READ_ENTITY(device_id, write_buffer, write_length, read_buffer, read_length);
    uint16_t reg = write_buffer[0] << 8 | write_buffer[1];
    bc_ppg_2c_read(device_id,reg,read_buffer,read_length);
//    GH3X2X_INFO_LOG("length2:0x%x 0x%x %d 0x%x 0x%x %d \r\n",device_id,write_buf_tmp,write_length,read_buffer[0],read_buffer[1],read_length);
    return ret;
}
...
//    GOODIX_PLATFORM_SPI_INIT_ENTITY();
    init_spi();
    am_bsp_gpio_pinconfig(AM_BSP_GPIO_SPICS_CTRL,g_AM_BSP_GPIO_IOM4_CS);
}
...
//      GOODIX_PLATFORM_SPI_WRITE_ENTITY(write_buffer, length);
    GH3X2X_INFO_LOG("length1: 0x%x 0x%x 0x%x %d \r\n",write_buffer[0],write_buffer[1],write_buffer[2],length);
    ret = spi_write((uint32_t*)write_buffer,length);
...
//    GOODIX_PLATFORM_SPI_READ_ENTITY();
    GH3X2X_INFO_LOG("length2: 0x%x 0x%x %d \r\n",read_buffer[0],read_buffer[1],length);
//    ret = spi_read(read_buffer[0],(uint32_t*)&read_buffer[1],length-1);
    spi_read((uint32_t*)read_buffer,length);
...
//    GOODIX_PLATFORM_SPI_CS_CTRL_ENTITY();
    am_bsp_gpio_write(AM_BSP_GPIO_SPICS_CTRL,cs_pin_level);
...
//   GOODIX_PLATFORM_SPI_RESET_PIN_ENTITY();
    bc_ppg_reset_io_control(pin_level);
}
...
//    am_util_debug_printf("----------------------------------------------------hal_gh3x2x_int_init \r\n");
...
{
//    am_util_debug_printf("----------------------------------------------------__NORMAL_INT_PROCESS_MODE__ \r\n");
...
#if (BC_ALG_SLEEP_VERSION == 1)
    app_gsensor_timer_start(GSENSOR_PPGACC_SAMPLE_TIME);
#endif
...
#if (BC_ALG_SLEEP_VERSION == 1)
    app_gsensor_timer_stop(GSENSOR_PPGACC_SAMPLE_TIME);
#endif
...
/**************************** WARNNING END*****************************************************/
	...
	{
		g_sensor_struct.ppg_g_sensor_data_read_callback((uint8_t*)gsensor_buffer,gsensor_buffer_index);
//    printf("hal_gsensor_drv_get_fifo_data:");
//    for(uint8_t i = 0;i < *gsensor_buffer_index;i++)
//    {
//      printf("%d %d  %d \n",gsensor_buffer[i].sXAxisVal,gsensor_buffer[i].sYAxisVal,gsensor_buffer[i].sZAxisVal);
//    }
//     printf("\r\n");
	}
...
//    am_util_debug_printf(log_string);
}
#endif
...
#if (defined HARD_VERSION_522) || (defined HARD_VERSION_523)
#if(CUS_VERSION == 2) || (CUS_VERSION == 5)
    app_ppg_setLedCur();      //set slot4 drv1 50 LSB
#endif
#endif
}
...
//    Gh3x2xSendData(uchTxDataBuf,usBufLen);
}
...
{
//    Gh3x2xSendTimerInit(uchPeriodMs);
}
