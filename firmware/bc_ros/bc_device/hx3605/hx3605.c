...
//ACC数据发送缓冲区
static int16_t accdata[20][3] = {0};
//心率实时数据存储
static int32_t hrsdata[20];
static uint32_t agc_green[20];
static uint8_t led_idac[20]; 
//血氧实时数据存储
static int32_t reddata[20];
static int32_t irdata[20];
static uint8_t red_idac[20]; 
static uint8_t ir_idac[20]; 
static uint16_t red_cur[20];     
static uint16_t ir_cur[20];    
static uint8_t datacnt = 0;


enum hx3605_ppg_time_type
{
	HX3605_PPG_TIMEOUT = 0,
	HX3605_PPG_AGC_TIMEOUT,
	HX3605_PPG_TIME_TYPE,
};
...
static bc_rtos_timer_struct  ppg_timer[HX3605_PPG_TIME_TYPE] = {
																   {
																	 .timer_name = "hx3605 timeout timer",
																	 .uxAutoReload = true,
																	 .xTimerPeriodInTicks = 320,
																	 .timer_callback_function = ppg_timeout_handler,   
																   },
																   {
																	 .timer_name = "agc_timeout timer",
																	 .xTimerPeriodInTicks = 40,
																	 .uxAutoReload = true,
																	 .timer_callback_function = agc_timeout_handler,   
																   }															   
																   
																 };

q_device_t *hx3605_i2c_ppg_dev;
static struct i2c_package i2c_pack = {
	                                     .slave_addr = 0x44 << 1  ,
                                     };

static void app_ppg_timer_start(enum hx3605_ppg_time_type time_id)
{
	bc_rtos_timer_start(ppg_timer[time_id].timer_handler,50);
}

static void app_ppg_timer_stop(enum hx3605_ppg_time_type time_id)
{
	bc_rtos_timer_stop(ppg_timer[time_id].timer_handler,50);
}


void hx3605_timer_init(void)
{	

    for(uint8_t i = 0;i < HX3605_PPG_TIME_TYPE; i++)
	{
		ppg_timer[i].timer_handler = bc_rtos_timer_create(ppg_timer[i].timer_name,
														  ppg_timer[i].xTimerPeriodInTicks,
														  ppg_timer[i].uxAutoReload, 
														   (void *)ppg_timer[i].timer_id,
															ppg_timer[i].timer_callback_function);
		if(ppg_timer[i].timer_handler != NULL)
		{
...
//	BC_LOG_INFO("kkkkkkkkkkkkk   %d ",en);
    if (en)
    {
		app_ppg_timer_stop(HX3605_PPG_TIMEOUT);
       app_ppg_timer_start(HX3605_PPG_TIMEOUT);
...
{
//	BC_LOG_INFO("kkrrrrrrrrrrrkkkkkk %d ",en);
    if (en)
    {
//        err_code = app_timer_start(m_agc_timer,APP_TIMER_TICKS(40),NULL);
		 app_ppg_timer_stop(HX3605_PPG_AGC_TIMEOUT);
		app_ppg_timer_start(HX3605_PPG_AGC_TIMEOUT);
...
uint8_t hx3605_get_id(void)
{
    return hx3605_read_reg(0x00);
}
...
{
    uint8_t fifo_num_temp = hx3605_read_reg(0x14)&0x7f;
...
bool hx3605_init(WORK_MODE_T mode,uint8_t wave_config,uint8_t seq)
{
    wave_upload_flag = wave_config;
    frame_seq = seq;
    data_seq = 0;
    work_mode_flag = mode;
    hx3605_data_reset();
...
//					bc_alg_check_wear_init(1);
        ...
        #endif
        #ifdef SPO2_ALG_LIB
        tyhx_spo2_para_usuallyadjust(SPO2_LOW_XCORR_THRE,
                                    SPO2_LOW_SNR_THRE,
                                    COUNT_BLOCK_NUM,
                                    SPO2_BASE_LINE_INIT,
                                    SPO2_SLOPE,
                                    SPO2_GSEN_POW_THRE);
        tyhx_spo2_para_barelychange(MEAN_NUM,
                                    SOP2_DEGLITCH_THRE,
                                    SPO2_REMOVE_JUMP_THRE,
                                    SPO2_LOW_CLIP_END_TIME,
                                    SPO2_LOW_CLIP_DN,
                                    SPO2_NORMAL_CLIP_DN,
                                    IR_AC_TOUCH_THRE,
                                    IR_FFT_POW_THRE,
...
void agc_timeout_handler(void * p_context)
{
//	BC_LOG_INFO("llttttttttttttttttttttttttttt\r\n");
    #ifdef TIMMER_MODE
    switch (work_mode_flag)
    {
        case HRS_MODE:
...
void hx3605_gesensor_Int_handle(void)
{
    ...
    {
//        qma6100_data_read(data,sizeof(data));
//		fml_g_sensor_event_handler(G_SENSOR_GET_DATA_EVENT,data);
        //            NRF_LOG_INFO ("x:%d-y:%d-z:%d",data[0],data[1],data[2]);//todo check worn status
    }
    ...
    gsen_fifo_x[15] = data[0] >> 1;
    gsen_fifo_y[15] = data[1] >> 1;
    gsen_fifo_z[15] = data[2] >> 1;
...
void hx3605_hrs_ppg_Int_handle(void)
{
    ...
    HRS_CAL_SET_T calR = get_hrs_agc_status();
...
//        bc_alg_rri_append(PPG_buf[ii]);
//        bc_alg_check_wear_append(PPG_buf[ii]);

        if(wave_upload_flag == 1)
        {
        #ifdef GSENSER_DATA
            accdata[datacnt][0] = gsen_fifo_x[ii];
            accdata[datacnt][1] = gsen_fifo_y[ii];
            accdata[datacnt][2] = gsen_fifo_z[ii];
        #endif
            hrsdata[datacnt] = PPG_buf[ii];
        //          LOG("%d/%d %d %d \r\n",1+ii,*count,datacnt,hrsdata[datacnt]);
            if(++datacnt >= *count){
//                services_info_send_hr_hrv_acc(data_seq,datacnt,hrsdata,accdata,frame_seq);
        ...
        else if(wave_upload_flag == 2)
        {
            ...
            if(++datacnt >= *count)
            {
//                services_info_send_hrv_alg_info(data_seq,datacnt,hrsdata,irdata,agc_green,led_idac,frame_seq);    //
                datacnt = 0;
            }
        }
    }
...
{ 
    ...
    *count = 8;
    ...
    for(ii=0;ii<*count;ii++)
    {
//        bc_alg_check_wear_append(red_buf[ii]);
...
#endif
            if(++datacnt >= *count){
              //  services_info_send_spo2_acc(data_seq,datacnt,reddata,irdata,accdata,frame_seq);    //????
        ...
        else if(wave_upload_flag == 2)
        {
            ...
            if(++datacnt >= *count){
//                services_info_send_spo2_alg_info(data_seq,datacnt,reddata,irdata,red_idac,ir_idac,red_cur,ir_cur,frame_seq);    //????
                datacnt = 0;
            }
        }
    }
...
bool hx3605_hr_data_callback_register(void *function_callback)
{
...
bool hx3605_spo2_data_callback_register(void *function_callback)
{
	if(function_callback == NULL)
	{
		return false;
...
bool hx3605_hr_result_callback_register(void *function_callback)
{
	if(function_callback == NULL)
	{
		return false;
...
bool hx3605_spo2_result_callback_register(void *function_callback)
{
	if(function_callback == NULL)
	{
		return false;
...
bool hx3605_spo2_signal_check_callback_register(void *function_callback)
{
	if(function_callback == NULL)
	{
		return false;
...
bool hx3605_hr_signal_check_callback_register(void *function_callback)
{
	if(function_callback == NULL)
	{
		return false;
