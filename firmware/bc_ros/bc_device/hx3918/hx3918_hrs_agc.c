...
void hx3918_hrs_ppg_init(void) //20200615 ericy ppg fs=25hz, phase3 conversion ready interupt en
{
    ...
    uint8_t leddrive_ps0 =10;     //LEDDRIVE_PS0  0~255 = 0~200ma
...
hx3918_hrs_wear_msg_code_t hx3918_hrs_wear_mode_check(int32_t infrared_data)
{
    ...
    {
        check_thre = hrs_ir_wear_thres + light_leak_val;
    }
    else
    {
        check_thre = hrs_ir_unwear_thres + light_leak_val;
    }
...
SENSOR_ERROR_T hx3918_hrs_enable(void)
{
    ...
    if (!chip_judge())
    {
        TYHX_LOG("check id failed!\r\n");
        return SENSOR_OP_FAILED;
    }
    else
    {
        TYHX_LOG("check id success!\r\n");
    }
    if (s_ppg_state)
    {
