...
int32_t LSM6DSO_Init(LSM6DSO_Object_t *pObj)
{
  ...
  /* FIFO mode selection */
  if (lsm6dso_fifo_mode_set(&(pObj->Ctx), LSM6DSO_STREAM_MODE) != LSM6DSO_OK)
  {
    return LSM6DSO_ERROR;
  }

  /* Select default output data rate. */
  pObj->acc_odr = LSM6DSO_XL_ODR_26Hz;
...
int32_t LSM6DSO_ACC_Enable_Pedometer(LSM6DSO_Object_t *pObj)
{
  ...
  lsm6dso_emb_sens_t emb_sens;
    ...
    //使能时间戳
    if(lsm6dso_timestamp_set(&(pObj->Ctx),1) != LSM6DSO_OK)
    {
        return LSM6DSO_ERROR;
    }
    ...
    //去抖
    uint8_t steps = 24;
    if(lsm6dso_pedo_debounce_steps_set(&(pObj->Ctx),&steps) != LSM6DSO_OK)
    {
        return LSM6DSO_ERROR;
    }
    
  /* Output Data Rate selection */
  if(LSM6DSO_ACC_SetOutputDataRate_With_Mode(pObj,LSM6DSO_XL_ODR_26Hz, LSM6DSO_ACC_LOW_POWER_NORMAL_MODE) != LSM6DSO_OK)
...
//  if (lsm6dso_pedo_sens_set(&(pObj->Ctx), LSM6DSO_FALSE_STEP_REJ_ADV_MODE) != LSM6DSO_OK)
  
  if(lsm6dso_pedo_steps_period_set(&(pObj->Ctx),150) != LSM6DSO_OK)
  {
      return LSM6DSO_ERROR;
  }
...
int32_t LSM6DSO_ACC_Enable_Wake_Up_Detection(LSM6DSO_Object_t *pObj, LSM6DSO_SensorIntPin_t IntPin)
{
  ...
  if (lsm6dso_wkup_threshold_set(&(pObj->Ctx), 0x10) != LSM6DSO_OK)
  {
    return LSM6DSO_ERROR;
  }
