...
uint8_t awritedata[20];
uint8_t areaddata[20];
uint8_t cnt = 0;
uint16_t mblength;
...
//I2C卸载
int NFC04A1_I2C_DeInit(void)
{
	bc_nfc_st25dv_i2c_close();
}

static int NFC04A1_I2C_IsReady(uint16_t DevAddr, uint32_t Trials)
{
    return NFCTAG_OK;
}

//读I2C
int NFC04A1_I2C_ReadReg16(uint16_t addr, uint16_t reg, uint8_t *pData, uint16_t size)
{
	bc_nfc_st25dv_i2c_read(addr,reg,pData,size);
	return NFCTAG_OK;
}

//写I2C
int NFC04A1_I2C_WriteReg16(uint16_t addr, uint16_t reg, uint8_t *pData, uint16_t size)
{

	bc_nfc_st25dv_i2c_write(addr,reg,pData,size);
	return NFCTAG_OK;
}

uint32_t NFC04A1_GetTick()
{
	uint32_t tick = bc_systick_get();
	return tick;
}
...
uint8_t MX_NFC_readID(void)
{
    uint8_t nfctag_id = 0;
    NFC04A1_NFCTAG_Init(NFC04A1_NFCTAG_INSTANCE);
    ST25DV_ReadID(&NfcTagObj, &nfctag_id);
    BC_LOG_INFO("nfctag_id = 0x%x \r\n",nfctag_id);
...
void MX_NFC_Init(void)
{
    ...
    for(uint8_t i = 0;i < 10;i++)
    {
        ...
        {
            BC_LOG_INFO("MX_NFC_Init failed");
        }
        else
        {
  ...
  if( MB_mode == ST25DV_DISABLE )
  {
   ...
   NFC04A1_NFCTAG_WriteMBWDG(NFC04A1_NFCTAG_INSTANCE,200);
...
//    NFC04A1_NFCTAG_WriteMailboxData( NFC04A1_NFCTAG_INSTANCE, awritedata, 20 );
//  }
//  NFC04A1_NFCTAG_ReadMailboxData(NFC04A1_NFCTAG_INSTANCE,areaddata,0,20);
//  LOG_DUMP_BYTE(awritedata,20);
//  LOG_DUMP_BYTE(areaddata,20);
 ...
 * @}
 */
//void MX_NFC4_MAILBOX_Process(uint8_t charing_status,uint8_t vbat_percen)
//{   
...
//    uint8_t len = 5;
//    
//    send_buf[1] = CMD_CHARGE_STATE;
//    send_buf[2] = 2;
//    send_buf[3] = vbat_percen;
//    send_buf[4] = charing_status;
//    //  初始化
////    memset(areaddata,0,i);
//    
...
//    //信箱为空，写入
//    NFC04A1_NFCTAG_ReadMBCtrl_Dyn( NFC04A1_NFCTAG_INSTANCE,&mbctrldynstatus );
//	 BC_LOG_INFO( "\n\rHost(i2c) Put Message     = %d    RF(reader) Put Message=%d \r\n", mbctrldynstatus.HostPutMsg ,mbctrldynstatus.RfPutMsg );

//    if( (mbctrldynstatus.HostPutMsg == 0) && (mbctrldynstatus.RfPutMsg == 0) )
//    {
////        LOG( "Mailbox available, write data to it \r\n" );
//        NFC04A1_NFCTAG_WriteMailboxData( NFC04A1_NFCTAG_INSTANCE, send_buf, len );
//        NFC04A1_NFCTAG_ReadMBCtrl_Dyn( NFC04A1_NFCTAG_INSTANCE,&mbctrldynstatus );
//      //  LOG_DUMP_BYTE(send_buf,len);
//		//NRF_LOG_HEXDUMP_INFO(send_buf,len);
//		BC_LOG_HEX_P("nfc send:",send_buf,len);
//    }
//    
//      /* Read mailbox status */
//    NFC04A1_NFCTAG_ReadMBCtrl_Dyn( NFC04A1_NFCTAG_INSTANCE,&mbctrldynstatus );
//    NFC04A1_NFCTAG_ReadMailboxData(NFC04A1_NFCTAG_INSTANCE,areaddata,0,len);
//  // NRF_LOG_HEXDUMP_INFO(areaddata,len);
////    LOG_DUMP_BYTE(send_buf,i);
//    BC_LOG_INFO( "\n\r\n\rCtrl MB status register value:" );
//    BC_LOG_INFO( "\n\rHost(i2c) Missed Message  = %d", mbctrldynstatus.HostMissMsg );
//    BC_LOG_INFO( "\n\rRF(reader) Missed Message = %d", mbctrldynstatus.RFMissMsg );
//    BC_LOG_INFO( "\n\rHost(i2c) Put Message     = %d", mbctrldynstatus.HostPutMsg );
...
//    BC_LOG_INFO( "\n\rMailbox Enable            = %d", mbctrldynstatus.MbEnable );
//    
////    LOG("MBCtrl_Dyn : %d %d %d %d %d \r\n",mbctrldynstatus.HostMissMsg,mbctrldynstatus.RFMissMsg,mbctrldynstatus.HostPutMsg,mbctrldynstatus.RfPutMsg,mbctrldynstatus.MbEnable);
//    
//}
