...
static void demoNfcv_recv( rfalNfcvListenDevice *nfcvDev );
 ...
 *****************************************************************************
 */
static bool set_wake_up_flag = true;
...
//			platformLog("nfc_state :%d \r\n",nfc_state);
            if( rfalNfcIsDevActivated( nfc_state ) )
            {
//				printf("nfc_state yyyyyyyyyy :%d \r\n",nfc_state);
...
//							demoNfcv_recv(&nfcDevice->dev.nfcv);
                        }
                        break;
...
//    rfalST25xVPollerReadConfiguration(reqFlag,uid,0x0d,&tmp);
//    platformLog("tmp = 0x%x \r\n",tmp);
//    rfalST25xVPollerReadDynamicConfiguration(reqFlag,uid,0x0d,&tmp);
//    platformLog("tmp = 0x%x \r\n",tmp);
//    rfalST25xVPollerReadMessageLength(reqFlag,uid,&tmp);
//    platformLog("tmp = 0x%x \r\n",tmp);
////    platformLog(" Read Block: %s %s\r\n", (err != RFAL_ERR_NONE) ? "FAIL": "OK Data:", (err != RFAL_ERR_NONE) ? "" : hex2Str( &rxBuf[1], DEMO_NFCV_BLOCK_LEN));

//    rfalST25xVPollerReadMessage(reqFlag,uid,mbPointer,numBytes,rxBuf,tmp+2,&rcvLen);
//    platformLog(" Read Block: %s %s\r\n", (err != RFAL_ERR_NONE) ? "FAIL": "OK Data:", (err != RFAL_ERR_NONE) ? "" : hex2Str( &rxBuf[1], DEMO_NFCV_BLOCK_LEN));
#endif /* RFAL_FEATURE_NFCV */
}
