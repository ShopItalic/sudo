...
void rfalDpoInitialize( void )
{
    ...
    RFAL_MEMCPY( gRfalDpo, gRfalCurrentDpo, sizeof(rfalDpoDefaultSettings) );
...
ReturnCode rfalDpoTableWrite( rfalDpoEntry* powerTbl, uint8_t powerTblEntries )
{
    ...
    /* copy the data set  */
    RFAL_MEMCPY( gRfalDpo, powerTbl, (powerTblEntries * RFAL_DPO_TABLE_PARAMETER) );
...
ReturnCode rfalDpoTableRead( rfalDpoEntry* tblBuf, uint8_t tblBufEntries, uint8_t* tableEntries )
{
    ...
    RFAL_MEMCPY( tblBuf, gRfalCurrentDpo, (tblBufEntries * RFAL_DPO_TABLE_PARAMETER) );
...
ReturnCode rfalDpoAdjust( void )
{
    ...
    if( RFAL_ERR_NONE != gRfalDpoMeasureCallback( &refValue ) )
    {
        return RFAL_ERR_IO;
    }
