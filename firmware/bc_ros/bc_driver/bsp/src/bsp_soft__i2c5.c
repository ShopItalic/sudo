...
/*******************************************************************************
此为bsp 软件模拟i2c3文件，通过宏定义来兼容nordic、phy6222硬件平台
...
static struct BSP_I2C bsp_list =   
{
	  ...
	  .name = "i2c_5",  //i2c0,i2c0为硬件i2c，i2c2之后为模拟i2c
	  .lock = false,
	  .i2c_config = {
			              .delay_length = 1,
#if (defined(HANDWARE_1_5_8) )
						  .i2c_io_scl_pin = NRF_GPIO_PIN_MAP(1,11),
						  .i2c_io_sda_pin = NRF_GPIO_PIN_MAP(1,14),
#elif (defined(HANDWARE_1_5_6) )
						  .i2c_io_scl_pin = NRF_GPIO_PIN_MAP(1,04),
						  .i2c_io_sda_pin = NRF_GPIO_PIN_MAP(0,15),	
#elif (defined(HANDWARE_1_9_1) )
						  .i2c_io_scl_pin = NRF_GPIO_PIN_MAP(0,03),
...
static struct BSP_I2C bsp_list =   
{
	  ...
	  .name = "i2c_3",  //i2c0,i2c0为硬件i2c，i2c2之后为模拟i2c
	  .lock = false,
	  .i2c_config = {
			              .delay_length = 2,
			              .i2c_io_scl_pin = 0,
 ...
 * Return            : 
 *******************************************************************************/
static uint8_t bsp_i2c_writeregs(uint8_t slave, uint16_t reg_add,uint8_t *reg_data,uint8_t length,struct simulation_i2c_config const *bsp_i2c)
{
	bsp_i2c_start(bsp_i2c);
	bsp_i2c_write_byte(slave,bsp_i2c);
	if(bsp_i2c_wait_ack(bsp_i2c)){
 ...
 * Return            : 
 *******************************************************************************/
static uint8_t bsp_i2c_readreg(uint8_t slave, uint16_t reg_add,uint8_t *buf,uint8_t num,struct simulation_i2c_config const *bsp_i2c)
{
	bsp_i2c_start(bsp_i2c);
	bsp_i2c_write_byte(slave,bsp_i2c);
	if(bsp_i2c_wait_ack(bsp_i2c)){
 ...
 * Return            : 
 *******************************************************************************/
static void bsp_simulation_i2c5_register(void)
{
...
device_initcall(bsp_simulation_i2c5_register);
