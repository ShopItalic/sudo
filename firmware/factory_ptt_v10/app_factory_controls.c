...
bool app_factory_controls_command(const uint8_t *d, unsigned n)
{
    ...
    if (d && n >= 4U && d[2] == 0x36U && factory_capture_active()) {
        ...
        /* The status probe returns an enum: zero means IDLE, not failure.
         * Keep it truthful through recording, rollover and finalization so
         * the phone can defer cleanup instead of attempting a rejected delete. */
        if (d[3] == 0x17U) busy[4] = 3U; /* PPG_FLS_BUSY */
