...
#define MAGIC 0x03435446UL /* P06: high byte = haptics, light mute, delay index */
#define LIGHT_MUTED 0x02000000UL
#define HOLD_DELAY_MASK 0xfc000000UL
#define LEGACY_VALUES_MASK 0x01ffffffUL
#define DEFAULTS (0x01000001UL | ((uint32_t)(FACTORY_HOLD_DEFAULT_STEPS - 1U) << 26))
...
static void reply(const uint8_t *r, unsigned result)
{
    ...
    put32(packet + 14, current[2] & LEGACY_VALUES_MASK);
...
static void load_record(void)
{
     ...
     * The first actual settings change saves schema 3. */
    if ((current[0] != MAGIC && current[0] != LIGHT_MAGIC && current[0] != LEGACY_MAGIC) ||
        (current[0] == LEGACY_MAGIC && (current[2] >> 24) > 1U) ||
        (current[0] == LIGHT_MAGIC && (current[2] >> 24) > 3U) ||
        !valid_values(current[2], PROTOTYPE_HOLD_MAX_STEPS) || current[3] != crc(current)) storage_fault = true;
...
bool app_factory_controls_command(const uint8_t *d, unsigned n)
{
    ...
    if (n < 9U) return true;
    if (d[0] != 0U || (d[4] < 1U || d[4] > 3U) || n != (d[3] == 0x10U ? 9U : 16U + d[4])) { reply(d, INVALID); return true; }
    ...
    values = get32(d + 13);
    if (!valid_values(values, FACTORY_HOLD_MAX_STEPS) || (values >> 24) > 1U ||
        (d[4] >= 2U && d[17] > 1U) ||
        (d[4] == 3U && (d[18] < FACTORY_HOLD_MIN_STEPS || d[18] > FACTORY_HOLD_MAX_STEPS))) { reply(d, INVALID); return true; }
    /* Older schema writes preserve preferences absent from their packet. */
    values |= d[4] >= 2U ? (d[17] ? 0U : LIGHT_MUTED) : (current[2] & LIGHT_MUTED);
    values |= d[4] == 3U ? ((uint32_t)(d[18] - 1U) << 26) : (current[2] & HOLD_DELAY_MASK);
