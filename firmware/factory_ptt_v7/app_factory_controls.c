...
#define MAGIC 0x04435446UL /* P07: low-byte bits 4..7 additionally mute swipes */
...
static void load_record(void)
{
    ...
    if ((current[0] != MAGIC && current[0] != HOLD_MAGIC && current[0] != LIGHT_MAGIC && current[0] != LEGACY_MAGIC) ||
        (current[0] != MAGIC && (current[2] & SWIPE_MUTED_MASK) != 0U) ||
...
bool app_factory_controls_command(const uint8_t *d, unsigned n)
{
    ...
    if (n < 9U) return true;
    if (d[0] != 0U || (d[4] < 1U || d[4] > 4U) || n != (d[3] == 0x10U ? 9U : (d[4] >= 3U ? 19U : 16U + d[4]))) { reply(d, INVALID); return true; }
    ...
    values |= d[4] == 4U ? ((~(uint32_t)d[18]) & SWIPE_MUTED_MASK) : (current[2] & SWIPE_MUTED_MASK);
