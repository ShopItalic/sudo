...
static uint32_t factory_motor_active_since;
static uint32_t factory_motor_last_finished;
static bool factory_motor_has_finished;
...
static void factory_motor_stop_locked(void)
{
    ...
    if (notify) {
        factory_motor_last_finished = (uint32_t)xTaskGetTickCount();
        factory_motor_has_finished = true;
    }
...
static bool factory_motor_begin_locked(uint32_t *generation)
{
    ...
    *generation = ++factory_motor_generation;
    ...
    factory_motor_active_since = (uint32_t)xTaskGetTickCount();
...
void bc_linear_motor_activity_get(bc_linear_motor_activity_t *activity)
{
    if (!activity) return;
    taskENTER_CRITICAL();
    activity->generation = factory_motor_generation;
    activity->active_since_tick = factory_motor_active_since;
    activity->last_finished_tick = factory_motor_last_finished;
    activity->active = factory_motor_live;
    activity->has_finished = factory_motor_has_finished;
    taskEXIT_CRITICAL();
}
