...
#include "factory_swipes.inc"
static void set_swipes(uint32_t revision, uint32_t values, uint8_t light, uint8_t steps, uint8_t enabled) {
    uint8_t d[19] = {0,46,0x84,0x11,4,1,2,3,4};
    put32(d+9,revision); put32(d+13,values); d[17]=light; d[18]=(uint8_t)((enabled<<4)|steps);
    assert(app_factory_controls_command(d,19));
}
int main(void) {
    ...
    get[4]=5; app_factory_controls_command(get,9); CHECK(last_reply[9]==INVALID);
    ...
    for(unsigned step=11;step<=20;++step) {
        disk[0]=HOLD_MAGIC; disk[1]=17; disk[2]=0x02020201 | ((step-1U)<<26); disk[3]=crc(disk);
    ...
    /* Every combination uses the actual generated factory swipe dispatch. */
    for (unsigned mask=0; mask<16; ++mask) {
        for (unsigned steps=1; steps<=10; ++steps) {
            sample(0,0); set_swipes(current[1],0x00020201,0,(uint8_t)steps,(uint8_t)mask);
            if(saving) complete(0);
            CHECK(last_reply[9]==OK && last_reply[19]==((mask<<4)|steps));
            boot(); sample(0,0);
            CHECK(app_factory_controls_action(0)==1 && app_factory_controls_action(1)==2 && app_factory_controls_action(2)==2);
            CHECK(!app_factory_controls_haptics() && !app_factory_controls_recording_light());
            CHECK(app_factory_controls_hold_delay_ms()==steps*500U);
            for(unsigned direction=0;direction<4;++direction) {
                unsigned before=swipe_events[direction]; dispatch_swipe(direction);
                CHECK(swipe_events[direction]==before+((mask>>direction)&1U));
            }
            CHECK(!app_factory_controls_swipe_enabled(4));
        }
    }
    sample(0,0); set_swipes(current[1],0x00020201,0,10,5); complete(0);
    /* All older schemas preserve the swipe mask and hide packed mute flags. */
    set(current[1],0x01020201); complete(0); CHECK(last_reply[14]==1);
    set_light(current[1],0x01020201,1); complete(0); CHECK(last_reply[14]==1);
    set_delay(current[1],0x01020201,1,2); complete(0);
    CHECK(last_reply[19]==2 && last_reply[14]==1);
    get[4]=4; app_factory_controls_command(get,9); CHECK(last_reply[19]==0x52);
    /* Pending and failed saves cannot change the confirmed swipe switches. */
    set_swipes(current[1],0x01020201,1,2,10); CHECK(saving);
    CHECK(app_factory_controls_swipe_enabled(0) && !app_factory_controls_swipe_enabled(1));
    boot(); sample(0,0); CHECK(app_factory_controls_swipe_enabled(0));
    set_swipes(current[1],0x01020201,1,2,10); complete(42);
    CHECK(last_reply[9]==STORAGE && app_factory_controls_swipe_enabled(0));
    boot(); sample(0,0);
    set_swipes(current[1]-1U,0x01020201,1,2,10); CHECK(last_reply[9]==CONFLICT);
    for(unsigned steps=11;steps<=15;++steps) {
        set_swipes(current[1],0x01020201,1,(uint8_t)steps,0); CHECK(last_reply[9]==INVALID);
    }
    set_swipes(current[1],0x01020201,1,0,15); CHECK(last_reply[9]==INVALID);
    set_swipes(current[1],0x010202f1,1,2,15); CHECK(last_reply[9]==INVALID);
    sample(1,1); set_swipes(current[1],0x01020201,1,2,15); CHECK(last_reply[9]==BUSY);
    sample(0,0); app_factory_ptt_worker_service();
    for(unsigned n=0;n<24;++n) {
        uint8_t data[24]={0,1,0x84,0x11,4}; unsigned before=writes;
        app_factory_controls_command(data,n); CHECK(writes==before);
    }
    /* Fresh Ring and every old schema keep factory swipes enabled, no boot write. */
    flash_exists=false; old_writes=writes; boot();
    CHECK(writes==old_writes && app_factory_controls_hold_delay_ms()==500);
    for(unsigned direction=0;direction<4;++direction) CHECK(app_factory_controls_swipe_enabled(direction));
    flash_exists=true;
    for(unsigned schema=1;schema<=3;++schema) {
        disk[0]=(schema<<24)|0x00435446; disk[1]=21; disk[2]=0x01020201; disk[3]=crc(disk);
        boot(); CHECK(ready && writes==old_writes);
        for(unsigned direction=0;direction<4;++direction) CHECK(app_factory_controls_swipe_enabled(direction));
        disk[2]|=SWIPE_MUTED_MASK; disk[3]=crc(disk); boot(); CHECK(storage_fault && !ready);
    }
    disk[0]=MAGIC;
    ...
    for(unsigned direction=0;direction<4;++direction) CHECK(!app_factory_controls_swipe_enabled(direction));
    CHECK(!critical);
    printf("PASS P07 controls: %u checks; async persistence, reboot, corruption, mute, all gestures, busy, CAS, failures, malformed packets\n",checks);
    return 0;
}
