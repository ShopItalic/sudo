...
import prepare_factory_ptt_v7 as recipe

class FactoryControlsV7(unittest.TestCase):
...
""" + helper + '\n'
        ...
        fixture += '} }\n'
        touch = recipe.patch_touch(factory.vendor(recipe.APP + 'app_touch_button_handler.c'))
        swipe_fixture = """
#define BC_LOG_INFO(...) ((void)0)
enum { BUTTON_SWIPE_UP_PRESS=4, BUTTON_SWIPE_DOWN_PRESS=5, BUTTON_SWIPE_LEFT_PRESS=6, BUTTON_SWIPE_RIGHT_PRESS=7 };
static unsigned swipe_events[4];
static void app_package_button_up(unsigned button) { assert(button>=4 && button<=7); ++swipe_events[button-4]; }
static void dispatch_swipe(unsigned direction) { switch(direction) {
"""
        for index, direction in enumerate(('UP', 'DOWN', 'LEFT', 'RIGHT')):
            marker = '    else if((event_bits & TOUCH_SWIPE_'+direction+'_EVENT) == TOUCH_SWIPE_'+direction+'_EVENT)'
            block = touch.split(marker)[1].split('    else if')[0]
            swipe_fixture += 'case '+str(index)+': '+block+' break;\n'
        swipe_fixture += '} }\n'
        with tempfile.TemporaryDirectory() as folder:
            out=Path(folder)
            (out/'factory_recording_led.inc').write_text(fixture, encoding='latin1')
            (out/'factory_swipes.inc').write_text(swipe_fixture, encoding='latin1')
            command=[os.environ.get('CC','cc'),'-std=c99','-Wall','-Wextra','-Werror','-Wno-invalid-utf8','-O1','-g',
                ...
                '-Itests/firmware/factory_ptt_v2','-Ifirmware/factory_ptt_v7',
                'firmware/factory_ptt_v7/app_factory_ptt.c','tests/firmware/test_factory_controls_v7.c','-o',str(out/'controls')]
            ...
            for direction in ('UP','DOWN','LEFT','RIGHT'):
                (out/'factory_swipes.inc').write_text(swipe_fixture.replace(
                    'app_factory_controls_swipe_enabled(FACTORY_SWIPE_'+direction+')', '1'),encoding='latin1')
                subprocess.run(command,cwd=recipe.ROOT,check=True)
                negative=subprocess.run([str(out/'controls')],capture_output=True,text=True)
                self.assertNotEqual(negative.returncode,0,direction)
                self.assertIn('FAIL ',negative.stderr)
            print('PASS negative controls: all four missing swipe gates detected')
