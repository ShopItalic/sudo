"""Exercise P05 with the actual IQS driver, recorder lifecycle and shared file guards."""
...
from prepare_factory_ptt_v5 import IQS, APP, QUEUE, OVERLAY, PATCHES, patch_iqs, patch_touch, patch_ble, patch_queue
...
class FactoryPTTV5(unittest.TestCase):
    def test_generated_integration(self):
        with tempfile.TemporaryDirectory(prefix='factory-ptt-p05-') as folder:
            out = Path(folder)
            ...
            sources = [out / 'app_factory_ptt.c', out / 'IQS7211E.c', ROOT / 'tests/firmware/test_factory_ptt_v5.c']
            def run(name):
            ...
            print('PASS 8 negative controls: delayed tap raw latch, default triple notification, disconnected sensor, disabled lease, wrong-task recorder access, missing command handoff, tap cancels hold, tap leaks host notification')
