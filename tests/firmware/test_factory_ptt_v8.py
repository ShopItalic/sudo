"""Exercise P08 with the actual IQS driver, recorder lifecycle and shared file guards."""
...
from prepare_factory_ptt_v8 import IQS, APP, QUEUE, OVERLAY, PATCHES, patch_iqs, patch_touch, patch_ble, patch_queue
...
class FactoryPTTV8(unittest.TestCase):
    def test_generated_integration(self):
        with tempfile.TemporaryDirectory(prefix='factory-ptt-p08-') as folder:
            out = Path(folder)
            ...
            sources = [out / 'app_factory_ptt.c', out / 'IQS7211E.c', ROOT / 'tests/firmware/test_factory_ptt_v7.c']
            def run(name):
    ...
    def test_factory_delta(self):
        ...
        self.assertIn(APP + 'app_pdm_handler.c', PATCHES)
        self.assertIn(APP + 'app_ppg_file_data_handler.c', PATCHES)
