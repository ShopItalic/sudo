#!/usr/bin/env python3
"""Apply P05 to a fresh manifest-verified Z62 extraction. Never flash."""
import argparse
import json
...
VERSION = '6.0.3.3P05'
OVERLAY = ROOT / 'firmware/factory_ptt_v5'
...
PATCHES.update({APP+'app_touch_button_handler.c': patch_touch,
                ...
                BLE: patch_init, MOTOR: patch_motor,
...
def apply_overlay(destination, baseline):
    ...
    group.find('GroupName').text = 'Factory PTT P05 controls'
    ...
    project = old_project.with_name('factory_ptt_p05_build_only.uvprojx')
    tree.write(project, encoding='utf-8', xml_declaration=True)
    # The helper-created intermediate lacks the controls compilation unit.
    # Keep only the complete P05 build entry point, not a misleading P03 one.
    ...
    result.update(project=str(project.relative_to(destination)), projectSha256=hashlib.sha256(project.read_bytes()).hexdigest(),
                  ...
                  settings={'command': '0x84', 'get': '0x10', 'set': '0x11', 'schema': 1,
                            'fdsFile': '0x1001', 'fdsKey': '0x0001', 'hapticsDefault': True})
    (destination/'ptt-preparation.json').write_text(json.dumps(result, indent=2)+'\n')
    (destination/'NON-FLASHABLE.txt').write_text('Unsigned P05 experiment. Physical qualification pending.\n')
    return result
