#!/usr/bin/env python3
"""Apply P07 to a fresh manifest-verified Z62 extraction. Never flash."""
import argparse
import json
...
VERSION = '6.0.3.3P07'
OVERLAY = ROOT / 'firmware/factory_ptt_v7'
...
def apply_overlay(destination, baseline):
    ...
    group.find('GroupName').text = 'Factory PTT P07 controls'
    ...
    project = old_project.with_name('factory_ptt_p07_build_only.uvprojx')
    tree.write(project, encoding='utf-8', xml_declaration=True)
    # The helper-created intermediate lacks the controls compilation unit.
    # Keep only the complete P07 build entry point, not a misleading P03 one.
    ...
    (destination/'NON-FLASHABLE.txt').write_text('Unsigned P07 experiment. Physical qualification pending.\n')
    return result
