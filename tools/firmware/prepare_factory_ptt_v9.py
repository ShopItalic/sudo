#!/usr/bin/env python3
"""Apply P09 to a fresh manifest-verified Z62 extraction. Never flash."""
import argparse
import json
...
import factory_short_v9 as short
import factory_scroll_v9 as scroll
from prepare_vendor_baseline import prepare, ROOT

VERSION = '6.0.3.3P09'
OVERLAY = ROOT / 'firmware/factory_ptt_v9'
...
def patch_delete_file(s):
    ...
    replacement = '''/* P09: explicit commands only; no deferred filename-based erasure. */
...
def patch_ble(s):
    ...
    s = once(s, 'app_factory_ptt_before_command(ble_recv_msg.data,ble_recv_msg.data_length);\n\t\t\tapp_cmd_package_parse(ble_recv_msg.data,ble_recv_msg.data_length);',
             ...
             '                app_cmd_package_parse(ble_recv_msg.data,ble_recv_msg.data_length);\n            }')
    s = '#include "app_factory_scroll.h"\n' + s
    return s.replace('app_factory_ptt_worker_service();',
                     'app_factory_ptt_worker_service();\n            app_factory_scroll_service();')
...
PATCHES.update({APP+'app_touch_button_handler.c': patch_touch,
                ...
                BLE: lambda s: scroll.patch_ble(patch_init(s)), scroll.GAP: scroll.patch_gap,
                MOTOR: patch_motor, LED: patch_recording_light,
                APP+'app_cmd_handler.c': patch_delete_command,
                APP+'app_pdm_handler.c': lambda s: short.patch_pdm(local_recording.patch_pdm(s)),
                'firmware/bc_ros/bc_module/file/LittleFS/lfs_port.c': short.patch_lfs,
                'firmware/bc_ros/bc_module/spi_flash/bc_spi_flash_port.c': short.patch_spi,
...
def apply_overlay(destination, baseline):
    ...
    group.find('GroupName').text = 'Factory PTT P09 controls'
    for name in ('app_factory_controls.c', 'app_factory_controls.h', 'app_factory_motor.h',
                 'app_factory_delete.c', 'app_factory_delete.h') + battery.ADDED + short.ADDED + scroll.ADDED:
    ...
    project = old_project.with_name('factory_ptt_p09_build_only.uvprojx')
    tree.write(project, encoding='utf-8', xml_declaration=True)
    # The helper-created intermediate lacks the controls compilation unit.
    # Keep only the complete P09 build entry point, not a misleading P03 one.
    ...
    result.update(project=str(project.relative_to(destination)), projectSha256=hashlib.sha256(project.read_bytes()).hexdigest(),
                  ...
                  localRecordingFunctionsUnmodified=False, savedFileUploadWorkerUnmodified=False,
                  savedFileUploadPayloadBodyUnmodified=True,
                  ...
                  deletion={'stage': 'bounded-command-hardening', 'command': '0x36/0x12',
                            ...
                            'shortRecordingFilter': {'minimumHalfSeconds': 0, 'maximumHalfSeconds': 10, 'defaultHalfSeconds': 4, 'get': '0x13', 'set': '0x14', 'fdsKey': '0x0002'},
                            'alreadyAbsentIsSuccess': True, 'postDeleteAbsenceCheck': True,
                            'checkedFlashErrors': True, 'mountFailureAutoformat': False,
                            'lowSpaceReclamation': False,
                            'releaseBlockedBy': ['persistent recording identity and receiver receipt for transfer cleanup',
                  ...
                  scrolling={'transport': 'encrypted BLE HID mouse', 'reportBytes': 5,
                             'wheelAndPan': True, 'gestureNotificationsReplaced': True,
                             'queueLimit': 4, 'eventExpiryMs': 250, 'replayAfterReconnect': False,
                             'physicalPhoneQualification': False},
                  settings={'command': '0x84', 'get': '0x10', 'set': '0x11', 'schema': 5, 'legacySchemasSupported': [1, 2, 3, 4],
                            'swipesDefaultEnabled': [], 'legacySwipeMigration': 'off-until-schema-5-save',
    ...
    (destination/'NON-FLASHABLE.txt').write_text('Unsigned P09 experiment. Physical qualification pending.\n')
    return result
