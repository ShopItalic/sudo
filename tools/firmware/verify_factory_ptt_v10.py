#!/usr/bin/env python3
"""Verify P10 factory PTT source isolation and ArmCC5 load bounds; never flash."""
import argparse
import hashlib
import json
...
ALLOWED = {
    'BCL603S2X/app/components/libraries/sha256/sha256.c',
    'bc_ros/bc_application/app_package.c',
    'bc_ros/bc_module/file/LittleFS/lfs_port.c',
    'bc_ros/bc_module/spi_flash/bc_spi_flash_port.c',
    ...
    'bc_ros/bc_module/ble/src/bc_ble_gap.c',
    ...
    'bc_ros/bc_module/pmic/bc_power.c',
    'bc_ros/bc_application/app_pmic_handler.c',
    'bc_ros/bc_driver/bsp/src/bsp_adc.c',
}
...
def verify(source, elf_path, binary_path):
    ...
    for relative, item in overlays.items():
        ...
        if item.get('added') and sha((ROOT / 'firmware/factory_ptt_v10' / Path(relative).name).read_bytes()) != item['sha256']:
            raise ValueError('Active overlay differs from compiled source: ' + relative)
    # The generated adapters must match the current recipe too. Matching an old
    # preparation receipt alone cannot qualify later edits to the active code.
    import prepare_factory_ptt_v10 as recipe
    for path, patch in recipe.PATCHES.items():
        original = subprocess.check_output(['git', 'show', '102bfd2:' + path], cwd=ROOT)
        expected = patch(original.decode('latin1').replace('\r\n', '\n')).replace('\n', '\r\n').encode('latin1')
        if (source / path).read_bytes() != expected:
            raise ValueError('Active recipe differs from compiled source: ' + path)
    ...
    for name, budget in (('app_touch_event_handler_thread', 1024),
                         ('app_ble_recv_handler_thread', 4096),
                         ('app_pdm_handler_thread', 2048),
                         ('app_pdm_irq_handler_thread', 16384),
                         ('bc_ic_led_handler_thread', 512),
                         ('ppg_file_data_upload_handler_thread', 4096),
                         ('ppg_file_resume_upload_handler_thread', 4096),
                         ('ppg_file_one_click_upload_handler_thread', 4096)):
