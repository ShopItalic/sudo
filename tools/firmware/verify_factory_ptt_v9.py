#!/usr/bin/env python3
"""Verify P09 factory PTT source isolation and ArmCC5 load bounds; never flash."""
import argparse
import hashlib
import json
...
def verify(source, elf_path, binary_path):
    ...
    for relative, item in overlays.items():
        ...
        if item.get('added') and sha((ROOT / 'firmware/factory_ptt_v9' / Path(relative).name).read_bytes()) != item['sha256']:
            raise ValueError('Active overlay differs from compiled source: ' + relative)
    binary = binary_path.read_bytes()
    if binary.count(b'6.0.3.3P09\0') != 2 or b'6.0.3.3Z62\0' in binary:
